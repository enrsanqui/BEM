...
!	READ(1,*) Nel,(BCode(Nel,m),m=1,Ndofe)
	...
	Nodes: &
	DO M= 1,Nodel
		WRITE(2,*) 'Node=',M,Elres_u(Nel,na:na+ndof-1)
		Na= na+Ndof
	END DO &
	Nodes
END DO &
...
Elem_presc_trac:  &
DO n=1,NE_t
	READ(1,*,IOSTAT= ios) Nel,(Elres_t(Nel,m),m=1,Ndofe)						
	WRITE(2,*)'Element ',Nel,'  Prescribed values: '
	Na= 1
	Nodes1: &
	DO M= 1,Nodel
		WRITE(2,*) 'Node=',M,Elres_t(Nel,na:na+ndof-1)
		Na= na+Ndof
	END DO &
	Nodes1
END DO &
