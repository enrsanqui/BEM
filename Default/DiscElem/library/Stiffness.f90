...
CONTAINS
SUBROUTINE Stiffness_BEM(nr,xP,xP_Cont,Nodel,Ndof,Ndofe,NodeR,Ncode,NdofR,Ndofc,KBE,A,tc,Cdim,Elres_u,Elres_t,&
												IncieR,Incie_Cont,LdesteR,Nbel,ListR,TypeR,Bcode,Con,E,ny,Ndest,Isym)
!---------------------------------------------
...
!--------------------------------------------
IMPLICIT NONE
REAL, INTENT(INOUT):: xP(:,:)    !  Array of coordinates of collocation nodes
REAL, INTENT(INOUT):: xP_Cont(:,:)    !  Array of node coordinates
...
!     Scaling
!------------------------------------------
CALL Scal_Discont(E,xP_Cont,xP,Elres_u,Elres_t,Cdim,Scad,Scat)
!----------------------------------------------------------------
...
DO Nel=1,Nbel(nr)
		ne= ListR(nr,Nel)
		Elcor(:,:)= xP_Cont(:,Incie_Cont(ne,:))      !    gather element coords
		IF(Cdim == 2) THEN
			IF(Ndof == 1) THEN
...
!				CALL Integ2E(Elcor,IncieR(ne,:),Nodel,NodeR(nr),xP,E,ny,dUe,dTe,Ndest,Isym)  
				CALL Integ2E_Disc(Elcor,IncieR(ne,:),Nodel,NodeR(nr),xP,E,ny,dUe,dTe,Ndest,Isym)
			END IF
		ELSE
