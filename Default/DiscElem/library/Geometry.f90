...
SUBROUTINE Normal_Jac_disc(v3,Jac,xsi,eta,ldim,nodes,inci,coords)
!--------------------------------------------------------
!   Computes normal vector and Jacobian
!   at point with local coordinates xsi,eta
!--------------------------------------------------------
...
ALLOCATE (DNi(nodes,ldim),V1(Cdim),V2(Cdim))
!    Compute derivatives of shape function
Call ShapefunctionDisc_deriv(DNi,xsi,eta,ldim,nodes,inci)
!    Compute vectors in xsi (eta) direction(s)
DO I=1,Cdim
	V1(I)= DOT_PRODUCT(DNi(:,1),COORDS(I,:))
	IF(ldim == 2) THEN
...
DEALLOCATE (DNi,V1,V2)
RETURN
END SUBROUTINE Normal_Jac_disc
...
Subroutine ShapefunctionDisc(Ni,xsi,eta,ldim,nodes,inci)
!---------------------------------
!   Computes discontinuous shape functions  Ni(xsi,eta)
!   for one and two-dimensional (linear/parabolic) finite boundary elements
!---------------------------------
REAL,INTENT(INOUT):: Ni(:)      ! Array with shape function values at xsi,eta
REAL, INTENT(IN):: xsi,eta    ! intrinsic co-ordinates
INTEGER,INTENT(IN):: ldim     ! element dimension
INTEGER,INTENT(IN):: nodes    ! number of nodes
INTEGER,INTENT(IN):: inci(:)  ! element incidences
REAL              :: mxs,pxs,met,pet   !  temporary variables
REAL              :: d1, d2  !  temporary variables
...
SELECT CASE (ldim)
  CASE (1)   !   one-dimensional element
    d1= 0.8; d2= 0.8;
    c1=1/(d1+d2);
    Ni(1)= c1 * ( d2 - xsi );  Ni(2)= c1 * ( d1 + xsi )
    IF(nodes == 2) RETURN  !  linear element finshed
    Ni(3)=  1./( d1 * d2 ) * ( d2 - xsi ) * (d1 + xsi )
    Ni(1)= Ni(1) - d2/( d1 + d2) * Ni(3)
    Ni(2)= Ni(2) - d1/( d1 + d2) * Ni(3)
  CASE(2)    !    two-dimensional element
...
END SELECT
RETURN
END SUBROUTINE ShapefunctionDisc
...
SUBROUTINE ShapefunctionDisc_deriv(DNi,xsi,eta,ldim,nodes,inci)
!---------------------------------
!   Computes derivatives of discontinuous shape functions Ni(xsi,eta)
!   for one and two-dimensional (linear/parabolic) finite boundary elements
!---------------------------------
IMPLICIT NONE
REAL,INTENT(OUT):: DNi(:,:)    ! Array with shape function derivatives at xsi,eta
...
SELECT CASE (ldim)
	CASE (1)   !   one-dimensional element
		...
		DNi(2,1)= c1
		IF(nodes == 2) RETURN  !  linear element finshed
		DNi(3,1)=  1/(d1*d2)*(d1-d2-2*xsi)
		DNi(1,1)= DNi(1,1) - d1*c1*DNi(3,1)
		DNi(2,1)= DNi(2,1) - d2*c1*DNi(3,1)
	CASE (2)   !    two-dimensional element
...
END SELECT
RETURN
END SUBROUTINE ShapefunctionDisc_deriv
...
SUBROUTINE ContToDiscont(Inci_Discont, Inci, xP_Discont, xP, Nodel, Maxe, Cdim)
!--------------------------------------------------------------------
!   Convert continuous to discontinuous elements
!---------------------------------------------------------------------
IMPLICIT NONE
REAL, INTENT (INOUT)		::  xP_Discont(:,:), xP(:,:)
INTEGER, INTENT (IN)		 ::	 Inci_Discont(:,:), Inci(:,:), Nodel, Maxe, Cdim
INTEGER								::	Incie(Nodel)
REAL		 							   ::  xsi, eta, Ni(Nodel),GCcor(Cdim), Elcor(Cdim,Nodel)
INTEGER								   ::  nel, nod

WRITE(2,*)"  "
WRITE(2,*)" Coordinates of discontinuous nodes "
WRITE(2,*)"  "
DO nel= 1, Maxe
	...
	DO nod=1, Nodel
		...
		END IF
		eta= 0
		CALL Serendip_func(Ni,xsi,eta,Cdim-1,Nodel,Incie)
		CALL Cartesian(GCcor,Ni,Cdim-1,elcor)          ! Cart. coords of Gauss pt
		xP_Discont(:, Inci_Discont(nel,nod))= GCcor
		WRITE(2,*)Inci_Discont(nel,nod),GCcor
	END DO
END DO

END SUBROUTINE ContToDiscont

SUBROUTINE Transform_DiscToCont(Elres,Elres_New,Incie,Nodel,Cdim)
!--------------------------------------------------------------------
!   Calculate displacements/tractions to element nodes
!---------------------------------------------------------------------
IMPLICIT NONE
REAL, INTENT (INOUT)		::  Elres(:),Elres_New(:)
INTEGER, INTENT (IN)		 ::	 Incie(:), Nodel, Cdim 
REAL		 							   ::  xsi, eta, Ni(Nodel)
INTEGER								   ::  nod, ldim,ndof, DoF, DoF1, nod1
ldim=Cdim-1
Elres_New=0.0
DO nod=1,Nodel
	IF(nod==1)xsi= -1.
	...
	IF(nod==3)xsi= 0.
	eta=0.0
	CALL ShapefunctionDisc(Ni,xsi,eta,ldim,Nodel,Incie)
	DO ndof=1, 2
		DoF=nod*2 -2+ ndof
		DO nod1=1, Nodel
			DoF1= (nod1-1) * 2  + ndof
			Elres_New(DoF)= Elres_New(DoF) + Elres(DoF1)*Ni(nod1)
		END DO
	END DO
END DO 
...
END SUBROUTINE Transform_DiscToCont
